...
{
    public partial class P_focusedBible_Debate_Original : Form
    {
        public P_focusedBible_Debate_Original(string player1 = "Player One", string player2 = "Player Two",
            int time2Answer = 20, int numRounds = 1, string difficulty = "Normal")
        {
 ...
 //          this.AddOwnedForm(Gamesettings); //indica que este va a ser el papa del form settings
        ...
        void StartAgan()
        {
            if ((round == numRounds && banner != "Round " + round) || ((countUp == noRepetir_PorDificultad.Length) && (difficulty != "All"))) // significa que el juego ha terminado
            {
                Timer_2Answer.Stop();
                sonido.Stop();
        ...
        void perder_Ganar()
        {
            ...
            if (lifes_1 == 0 || lifes_2 == 0 || ((countUp == noRepetir_PorDificultad.Length) && (difficulty != "All")))
            {
                ...
                if (round == numRounds) // si es el ultimo round
                {
                    Thread.Sleep(1500);
